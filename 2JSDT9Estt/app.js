...
App({
  ...
  pageDataInitial: function () {
            ...
            success: function (res) {
              ...
              _this.sendRequest({
                ...
                success: function (data) {
                  ...
                  pageInstance.setData(newdata)
                  let length = data.data.coupon_list.length;
                  if (data.data.coupon_list.length > 1) {
                    ...
                    _this.takeoutAniDown = setInterval(function () {
                      var query = wx.createSelectorQuery().in(_this.getAppCurrentPage())
                      query.select('#takeoutAnimation').boundingClientRect(function(res){
                        if(!res){
    ...
    if (pageInstance.cityLocationComps.length){
      for (let i in pageInstance.cityLocationComps){
        ...
        _this.sendRequest({
          hideLoading: pageRequestNum++ == 1 ? false : true,
          url: '/index.php?r=AppRegion/getAllExistedDataRegionList&is_xcx=1&form=' + param.form,
          success: function (data) {
            var newdata = pageInstance.data,
                id =  pageInstance.cityLocationComps[i];
  ...
  cartListPlus: function (event) {
    ...
    if (+modelid) {
      this._changeOrderCount(goodsid, newdata[compid + '.cartList.' + goodsid][modelid].num, modelid, function (cart_id) {
  ...
  reLocalAddress: function (event) {
    ...
    this.turnToPage('/pages/myAddress/myAddress?from=takeout&changeLocal=1&locateAddress=' + dataset.address);
  },
  ...
  _isPromotionPerson: function () {
    ...
    this.sendRequest({
      ...
      success: function (res) {
        res.data ? that.turnToPage('/pages/promotionUserCenter/promotionUserCenter') : that.turnToPage('/pages/promotionApply/promotionApply');
      }
    })
  },
  ...
  turnToVideoDetail : function(event) {
    ...
    this.turnToPage('/pages/videoDetail/videoDetail?detail=' + id);
  },
  // 处理数字
